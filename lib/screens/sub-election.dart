...
class SubElectionScreen extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return new Scaffold(
      appBar: new AppBar(
        backgroundColor: Color.fromRGBO(72, 121, 209, 1),
        title: Text("Select Sub-Election"),
...
class SubElection extends StatefulWidget {
  @override
...
class _ElectionState extends State<SubElection> {
  ...
  List<Widget> renderWidgets(ElectionViewModel viewModel) {
    ...
    viewModel.selectedElection.subElections.forEach((election){
         widgets.add(ListTile(
              leading: Icon(Icons.people),
              title: Text(election.electionName),
              trailing: Icon(Icons.arrow_forward),
              onTap: (){
                viewModel.navigateToHomeAction(election);
              },
            ));
    });
